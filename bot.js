...
async function fetchProfiles(uuid) {
  ...
  if (!r.data.success) throw new Error("Hypixel API error! In Minecraft type /api new, update HYPIXEL_API_KEY in Railway.");
...
async function fetchAH() { return (await axios.get("https://api.hypixel.net/v2/skyblock/auctions?page=0", HAPI())).data; }
...
async function calcNetworth(member, profile) {
  ...
  for (const { label, keys } of sects) {
    ...
    const valued = items.map(it => {
      ...
      if (it.stars >= 6) mult = 2.75 + (it.stars - 5) * 0.5;
  ...
  let petTotal = 0; const petV = [];
  for (const pet of pets) {
    const p = getPrice(prices, "PET_" + (pet.type||"").toUpperCase());
    if (p > 0) { petTotal += p; petV.push({ id:"PET_"+pet.type, name:pet.type+" (Lvl "+(pet.level||"?")+")", price:p, stars:0, recomb:false }); }
  }
  if (petTotal > 0) { totalItems += petTotal; categories.push({ label:"Pets", total:petTotal, items:petV.sort((a,b)=>b.price-a.price).slice(0,5) }); }
...
function stars(s) { if (!s) return ""; return " "+"\u272B".repeat(Math.min(s,5))+"\u2605".repeat(Math.max(0,s-5)); }
...
function getSkillXP(m,n){return m["experience_skill_"+n]||m?.player_data?.experience?.["SKILL_"+n.toUpperCase()]||0;}
...
client.on("interactionCreate",async interaction=>{
  ...
  try{
    ...
    if(cmd==="stats"){
      ...
      return interaction.editReply({embeds:[new EmbedBuilder()
        ...
        .addFields(
          ...
          {name:"\u2B50 Skill Avg",             value:apiOff?"API Off":skillAvg(member).toFixed(2), inline:true},
          {name:"\u2694\uFE0F Catacombs",       value:"Lvl "+dungLvl(cataXP),                      inline:true},
          {name:"\uD83D\uDCB0 Purse",           value:fmt(purse),                                   inline:true},
          {name:"\uD83C\uDFE6 Bank",            value:fmt(bank),                                    inline:true},
          {name:"\uD83D\uDDE1\uFE0F Slayer XP", value:fmt(sxp),                                    inline:true},
          ...
          {name:"\u2694\uFE0F Barb Rep",        value:fmt(member.nether_island_player_data?.barbarians_reputation||0), inline:true},
        )
    ...
    if(cmd==="networth"){
      ...
      for(const t of essTypes){const amt=ess[t]?.current||0;if(amt>0){essVal+=amt*150;essLines.push(t.charAt(0)+t.slice(1).toLowerCase()+": "+amt.toLocaleString());}}
      ...
      if(essVal>0) desc.push("\u26AB **Essence** — "+fmt(essVal)+(essLines.length?" ("+essLines.slice(0,3).join(", ")+(essLines.length>3?" +more":"")+")" :""));
      embed.setDescription(desc.join("\n"));

      // Categories — bold header, item list with stars
      for(const cat of nw.categories){
        if(!cat.total||cat.total<=0)continue;
        const lines=(cat.items||[]).map(it=>{
          const s=stars(it.stars||0);
          const rc=it.recomb?" \uD83E\uDDF1":"";
    ...
    if(cmd==="accessories"){
      if(sub==="budget"){
        ...
        if(!bought.length)return interaction.editReply("Budget too low. Cheapest is ~1,000 coins.");
      ...
      if(sub==="stats"){
        ...
        return interaction.editReply({embeds:[new EmbedBuilder().setTitle("Kuudra Stats - "+mojang.name+" on "+pn).setColor(0xFF6600).setThumbnail("https://mc-heads.net/avatar/"+mojang.id)
          .addFields(
            ...
            {name:"\uD83D\uDDE1\uFE0F Slayer Levels",value:"Rev **"+slayerLvl(sl.zombie?.xp||0,"zombie")+"** | Tara **"+slayerLvl(sl.spider?.xp||0,"spider")+"** | Sven **"+slayerLvl(sl.wolf?.xp||0,"wolf")+"**\nEnder **"+slayerLvl(sl.enderman?.xp||0,"enderman")+"** | Blaze **"+slayerLvl(sl.blaze?.xp||0,"blaze")+"** | Vamp **"+slayerLvl(sl.vampire?.xp||0,"vampire")+"**",inline:false},
