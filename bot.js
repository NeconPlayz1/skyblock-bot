...
const { getNetworth } = require("skyblock-networth");
...
const ACC = [
  ...
  { name:"Beacon 1",                 tier:"Common",    mp:3,  cost:10000,    cat:"Utility",  desc:"+5 stats beacon range" },
  { name:"Beacon 2",                 tier:"Uncommon",  mp:5,  cost:50000,    cat:"Utility",  desc:"+10 stats beacon range" },
  { name:"Beacon 3",                 tier:"Rare",      mp:8,  cost:200000,   cat:"Utility",  desc:"+15 stats beacon range" },
  { name:"Beacon 4",                 tier:"Epic",      mp:12, cost:1000000,  cat:"Utility",  desc:"+20 stats beacon range" },
  { name:"Beacon 5",                 tier:"Legendary", mp:16, cost:5000000,  cat:"Utility",  desc:"+25 stats beacon range" },
];
...
client.on("interactionCreate", async interaction => {
  ...
  try {
    ...
    /* /networth — Sky Miner style using skyblock-networth package */
    if (cmd === "networth") {
      const username = await resolveUser(interaction);
      const mojang   = await fetchMojang(username);
      const profiles = await fetchProfiles(mojang.id);
      ...
      let nwResult = null;
      try {
        nwResult = await getNetworth(member, { bankBalance: bank });
      } catch (e) {
        console.log("[NW] getNetworth error:", e.message);
      }

      const embed = new EmbedBuilder()
        .setColor(0x00AAFF)
        .setThumbnail("https://mc-heads.net/avatar/" + mojang.id)
        .setFooter({ text: "Powered by skyblock-networth" })
        .setTimestamp();

      if (nwResult && nwResult.networth > 0) {
        const total  = nwResult.networth    || 0;
        const unsoul = nwResult.unsoulbound || 0;
        ...
        // Category breakdown — Sky Miner style
        const types = nwResult.types || {};
        const catOrder = [
        ...
        for (const { key, label } of catOrder) {
          ...
          if (items.length > 0) {
            val += "\n" + items
              .map(it => "• " + (it.name || it.id || "Unknown") + " — " + fmt(it.price || 0))
              .join("\n");
          }
        ...
        embed.setDescription(
          ...
          "> Could not calculate item networth.\n" +
    ...
    if (cmd === "skills") {
      ...
      if (skillsDisabled(member)) {
        return interaction.editReply({ embeds:[new EmbedBuilder()
          ...
          .setThumbnail("https://mc-heads.net/avatar/" + mojang.id)
          .setDescription("**" + mojang.name + " has their Skills API turned off.**\n\nTo fix: Join Hypixel in Minecraft → type `/api` → enable **Skills API**")
          .setTimestamp()] });
      }
    ...
    if (cmd === "slayer") {
      ...
      return interaction.editReply({ embeds:[new EmbedBuilder()
        ...
        .setFooter({ text: totalXP===0 ? "All 0 — player may not have started slayer" : "Hypixel Skyblock Bot" })
        .setTimestamp()] });
    }
      ...
      if (sub === "stats") {
        ...
        const embed=new EmbedBuilder()
          ...
          .addFields(
            ...
            { name:"Skills", value:"Cata: **"+cataLvl+"** | Combat: **"+combatLvl+"** | Skill Avg: **"+avgSkill+"**", inline:false },
